...
export class FrameRenderer {
	...
	private videoTextureSource: TextureSource | null = null;
	...
	private layoutCache: {
		...
		maskRect: { x: number; y: number; width: number; height: number; sourceCrop: CropRegion };
	} | null = null;
	...
	async initialize(): Promise<void> {
		...
		try {
			if (canvas && "colorSpace" in canvas) {
				(canvas as unknown as { colorSpace: string }).colorSpace = "srgb";
			}
		} catch (error) {
	...
	async renderFrame(videoFrame: VideoFrame, timestamp: number): Promise<void> {
		...
		if (!this.videoSprite) {
			const texture = Texture.from(videoFrame as unknown as TextureSource);
			this.videoSprite = new Sprite(texture);
			this.videoTextureSource = texture.source;
			this.videoContainer.addChild(this.videoSprite);
			if (this.maskGraphics) {
				this.videoContainer.addChild(this.maskGraphics);
			}
		} else {
			this.videoTextureSource ??= this.videoSprite.texture.source;
			this.videoTextureSource.resource = videoFrame as unknown as TextureSource["resource"];
		...
		if (
			...
			if (!success) {
				throw new Error(
					"Failed to render annotations: a critical component (like a blur) could not be applied.",
				);
			}
		}
		...
		if (this.smoothedCursorState && this.compositeCtx && this.config.showCursor) {
			drawCursorOnCanvas(
				...
				this.smoothedCursorState,
				{
					...
					smoothingFactor: this.config.cursorSmoothing ?? DEFAULT_CURSOR_CONFIG.smoothingFactor,
					motionBlur: this.config.cursorMotionBlur ?? DEFAULT_CURSOR_CONFIG.motionBlur,
	...
	private updateAnimationState(timeMs: number): number {
		...
		return Math.max(
			...
			Math.abs(state.x - prevX) /
				Math.max(1, this.layoutCache?.stageSize.width || this.config.width),
			Math.abs(state.y - prevY) /
				Math.max(1, this.layoutCache?.stageSize.height || this.config.height),
		);
	}
