	...
	(
		...
		const endFocusDrag = (event: React.PointerEvent<HTMLDivElement>) => {
			...
			try {
				...
				// Failed to release pointer capture (expected in some edge cases)
			}
		};
		...
		useEffect(() => {
			...
			if (bgVideo) {
				if (isPlaying) {
					bgVideo.play().catch(() => {
						// Video play interrupted (harmless)
					});
				} else {
					bgVideo.pause();
				}
		...
		useEffect(() => {
			...
			(async () => {
				try {
					...
					console.warn("Native cursor assets are unavailable in preview.", error);
				}
