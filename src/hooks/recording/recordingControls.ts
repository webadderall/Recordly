...
import {
	...
	updateRecordingSessionFacts,
	selectMimeType,
} from "./captureSession";
import { getMicrophoneConstraints } from "./audioCapture";
import { finalizeRecordingSession, notifyRecordingFinalizationFailure, showRecordingFinalizationToast } from "./recordingFinalizer";
...
export async function startBrowserRecording(options: StartRecordingOptions) {
	...
	updateRecordingSessionFacts(options.refs, {
		...
		microphoneFallbackRequired: false,
	...
	if (wantsAudioCapture) {
		...
		const useLinuxPortal = options.selectedSource.id === "screen:linux-portal";
	...
	console.log(
		`Recording at ${width}x${height} @ ${frameRate ?? TARGET_FRAME_RATE}fps using ${mimeType ?? "browser default"} / ${Math.round(videoBitsPerSecond / BITS_PER_MEGABIT)} Mbps`,
	);

	options.refs.chunks.current = [];
	const hasAudio = options.refs.stream.current.getAudioTracks().length > 0;
	updateRecordingSessionFacts(options.refs, { hasEmbeddedAudio: hasAudio });
	...
	recorder.ondataavailable = (event) => {
		if (event.data && event.data.size > 0) {
			options.refs.chunks.current.push(event.data);
		}
	};
	recorder.onstop = async () => {
		await options.cleanupCapturedMedia();
		if (options.refs.chunks.current.length === 0) {
			return;
		}
		...
		try {
			...
			if (videoResult.path) {
				...
				await notifyRecordingFinalizationFailure(options.refs, "Failed to save the recording.");
			}
		} catch (error) {
			console.error("Error saving recording:", error);
			await notifyRecordingFinalizationFailure(
				options.refs,
				`Failed to finalize the recording. ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	};
	recorder.onerror = () => {
...
export async function pauseRecordingSession(options: {
	...
	if (!options.recording || options.paused) {
		return;
	}
